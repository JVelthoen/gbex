---
title: "Gradient boosting for extremes"
...
The gbex package allows to fit the conditional tail distribution with the use gradient boosting. Then using this fitted distribution the condtional extreme quantiles can be estimated. 

The main function in the package is the gbex function, which uses gradient boosting to fit the parameters of the generalized pareto distribution to data $(X_1,Y_1), \ldots (X_n,Y_n) $ to obtain the conditional extreme quantiles $Q_X(\tau)$ for $\tau$ close to $1$.

Let us first simulate some data
```{r}
  n = 1000  # number of observations
  d = 2     # dimensions of the predictor space
  X = data.frame(matrix(runif(n*d,-1,1),ncol=2))
  sigma =  apply(abs(X),1,sum) + 1
  gamma = apply(abs(X),1,sum)/(d+2) + 0.1
  y = sigma*(runif(n)^(-gamma)-1)/gamma
```
 
With gradient boosting we can fit the conditional distribution,
```{r}
  fit = gbex(y, X, B = 150,lambda_scale = 0.05,lambda_ratio = 3,gamma_positive=F,depth=c(1,1),silent=T)
```
...
depth=c(1,0) # Maximum depth of trees for sigma and gamma
```
