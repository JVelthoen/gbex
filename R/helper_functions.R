...
first_guess <- function(y){
  ...
  theta_init=matrix(rep(theta_init,n),ncol=2,byrow=T)
  colnames(theta_init)=c('s','g')
  return(theta_init)
}

#' For a gradient tree fill the leafnodes with the gradient update gamma
#'
...
#'
#' @param tree a gradient tree
#' @param DF_boost the boosting data.frame
#' @return A data.frame object with for each leaf the gradient update
fill_leafs_s <- function(tree,DF_boost){
  update_df <- data.frame(leaf = treeClust::rpart.predict.leaves(tree,newdata=DF_boost)) %>%
      dplyr::mutate(d1 = DF_boost$r_s, d2 = DF_boost$r2_s) %>%
      dplyr::group_by(leaf) %>% dplyr::summarise(d1=mean(d1),d2=mean(d2)) %>%
      dplyr::mutate(gamma = d1/d2) %>%
...
#' @param DF_boost the boosting data.frame
#' @return A data.frame object with for each leaf the gradient update
fill_leafs_g <- function(tree,DF_boost){
  update_df <- data.frame(leaf = treeClust::rpart.predict.leaves(tree,newdata=DF_boost)) %>%
    dplyr::mutate(d1 = DF_boost$r_g, d2 = DF_boost$r2_g) %>%
    dplyr::group_by(leaf) %>% dplyr::summarise(d1=mean(d1),d2=mean(d2)) %>%
    dplyr::mutate(gamma = d1/d2) %>%
    dplyr::select(gamma,leaf)
  return(update_df)
...
get_DF_boost <- function(DF,theta){
